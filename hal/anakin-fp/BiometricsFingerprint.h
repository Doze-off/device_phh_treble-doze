...
private:
    ...
    void enqueueGoodixCommand(unsigned int command, uint64_t sequence);
    ...
    // Keep the vendor-specific 200001/200002/200003 protocol serialized.
    // A plain command-only queue allowed a delayed command from the previous
    // press to execute during the next press and desynchronize Goodix state.
    std::mutex mCommandMutex;
    std::condition_variable mCommandCondition;
    std::deque<GoodixCommand> mCommandQueue;
    std::atomic<uint64_t> mFingerSequence{0};
    std::atomic<bool> mFingerDown{false};
    std::atomic<bool> mLightAreaStable{false};
};
